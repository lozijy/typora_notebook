...
不需要真的把最后几个元素给弄掉,只需要返回时把数字减小就行了，只会返回数组前几位.
...
![image-20220529140524068](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220529140524068.png)

思路

使用哈希表记录每一个字符串出现的次数,然后遍历target,对每一个字符在哈希表中查找其对应次数,如果哈希表中没有该字符或者出现次数为零，则返回false，否则修改哈希表中存储的次数-1；
...
![image-20220529141420187](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220529141420187.png)

思想:

dp,dp\[i][j]表示从0,0到达i-j位置需要移除的最少障碍物,初始条件为dp\[0][0]=grid\[0][0],状态转移方程为dp\[i][j]=

#### 导师![image-20220530003104213](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220530003104213.png)
...
public class Main {
        ...
        public static void main(String[] args) {
            ...
            for (int i = 0; i < 5; i++) {
                if (smallest[biggest[i]] == i) {
                    System.out.println(i+1+" "+biggest[i]+1+" "+arrs[i][biggest[i]]);
                }
            }
...
![image-20220530003038022](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220530003038022.png)
...
![image-20220530003010139](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220530003010139.png)

![image-20220530002848270](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220530002848270.png)
...
public class Main {
        public static void main(String[] args) {
            ...
            for (int i = 0; i < s1.length(); i++) {
                s2="";
                if(Character.isDigit(s1.charAt(i))&&!set.contains(i)){
                    l=i;
                    while(Character.isDigit(s1.charAt(l))){
                        s2+=s1.charAt(l);
                        l++;
                        set.add(l);
                    }
...
![image-20220530002823465](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220530002823465.png)
...
![image-20220530002757452](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220530002757452.png)
...
![image-20220530104107742](https://raw.githubusercontent.com/lozijy/github_-/main/image-20220530104107742.png)
...
public int sumRootToLeaf(TreeNode root) {
    int ans=0;
    backTrack(ans,root,new StringBuffer());
    return ans;
}
public void backTrack(int ans ,TreeNode cur,StringBuffer str){
    if(cur.left==null||cur.right==null){
        if(!str.equals("")&&str.length()!=0) {
            ans += Integer.parseInt(String.valueOf(str), 2);
        }
        return;
    }
    str.append(String.valueOf(cur.val));
    backTrack(ans,cur.left,str);
    str.deleteCharAt(str.indexOf(String.valueOf(cur.val)));
    backTrack(ans,cur.right,str);
}
```
